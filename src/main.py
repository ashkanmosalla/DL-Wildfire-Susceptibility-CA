...
def run_wildfire_pipeline(data_path, target_col):
    ...
    for fold in range(10):
        ...
        best_cfg = tuner.run_calibration(X_train, y_train, X_val, y_val)

        # Training final model for this fold with best_cfg...
        # Store results for Table 3
        results.append({'fold': fold, 'config': best_cfg})
...
if __name__ == "__main__":
    ...
    # CatBoost for this target uses depth=6, iterations=80 (Section 2.5).
    run_wildfire_pipeline('wildfire_frequency.csv', 'ndfb_kd_co')

    # Running for Likelihood (Target 2)
    # CatBoost for this target uses depth=8, iterations=30 (Section 2.5).
