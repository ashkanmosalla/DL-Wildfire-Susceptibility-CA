...
class SpatialCVManager:
    ...
    # Alias kept so the pipeline entry point (main.py) resolves to a single
    # implementation; get_split and get_fold_data are equivalent.
    def get_split(self, df, fold_id):
        """Alias for get_fold_data (see above)."""
        return self.get_fold_data(df, fold_id)
